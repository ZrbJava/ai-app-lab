...
import { ArtifactRenderer } from '@/components/artifact/ArtifactBlocks'
...
import { isArtifactPart, type LabUIMessage } from '@/lib/chat-types'
...
function renderPart(
	...
	if (isArtifactPart(part)) {
		return <ArtifactRenderer data={part.data} />
	}
...
const AGENT_MODE_KEY = 'lab08-agent-mode'
				...
				<div className='min-w-0'>
					...
					{agentMode && (
						<p className='mt-0.5 truncate text-xs text-violet-600'>
							Artifact：reasoning → thinking → task → answer
						</p>
					)}
				</div>
